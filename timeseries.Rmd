...
Because we have a monthly series, we check the significance of the potential calendar effects on our log_transformed series, which will be Easter and Trading Days over our model `mod1b`
...
- The 14th observation is identified as a transient change (TC) outlier, characterized by a notable statistical value |3.6|>2. In the logarithmic scale (our series being log-transformed), its magnitude is represented by Wcoeff=-0.12, indicating a decline in the total production of renewable energy in USA compared to what would have occurred without this atypical event. The impact of the TC outlier is evident in February 1991 during the Iraq War but diminishes relatively swiftly over subsequent periods, following an exponential decrease with a delta value of 0.7.
...
```{r}
(mod.lin.c=arima(lnserie.lin,order=c(1,1,1),seasonal=list(order=c(0,1,1),period=12)))
...
Nonetheless, before proceeding, we test for an extra MA seasonal lag and MA regular to see its effect and if we should extend or not `mod.lin.b`:

```{r}
(mod.lin.b_2=arima(lnserie.lin,order=c(4,1,0),seasonal=list(order=c(0,1,1),period=12)))
...
```{r}
(mod.lin.b_3=arima(lnserie.lin,order=c(3,1,0),seasonal=list(order=c(0,1,2),period=12)))
...
```{r}
(mod.lin.b_4=arima(lnserie.lin,order=c(4,1,0),seasonal=list(order=c(0,1,2),period=12)))

abs(mod.lin.b$coef/sqrt(diag(mod.lin.b$var.coef)))
cat("\nSignificant?:",abs(mod.lin.b$coef/sqrt(diag(mod.lin.b$var.coef)))>2)
```
And now, according to those results, adding an extra seasonal lag `sma2`, as in `model.lin.b_3` = B = (3,1,0)(0,1,2)_12 decreases the AIC with aic = -1367.04, improving the model rating. Because of that, we finally continue with `model.lin.b_3`.

## Validation of model fitted to the (log) linearized series

```{r}
model=mod.lin.b_3       #Fitted ARIMA model: in this case mod is ARIMA(0,1,2)(0,1,1)_12 for lnserie
validation(model)
```
**Normality and variance**

They are both much more accepted than before according to Shapiro-Wilk and Breusch-Pagan tests, and by Q-Q plots, histogram, square root of residuals. Residuals plot doesn't have outliers out of confidence interval, which it had in previous models.


**Invertibility and causality**

Modul of AR Characteristic polynomial Roots:  1.555798 2.249925 1.555798 
Modul of MA Characteristic polynomial Roots:  1.159602 1.015442 1.015442 1.015442 1.015442 1.015442 1.015442 1.015442 1.015442 1.159602 1.015442 1.159602 1.015442 1.015442 1.159602 1.015442 1.159602 1.159602 1.159602 1.159602 1.159602 1.159602 1.159602 1.159602
...
We have to arrive until lag 50 in the Ljung-Box test in order to fail lag independence, which is a considerable improvement compared to previous models. On the other hand, in the Durbin-Watson test the accepted hypothesis is that there is no evidence to suggest a first-order autocorrelation in the residuals, and they can be considered independent.

#### Linearized model stability
...
```
Because of the huge similarity in the coefficients and standard error we confirm the stability of the linearized model.

## Forecasting and prediction capability: linearized model
...
ts.plot(serie,tl,tu,pr,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=ultim[1]+c(-3,+2),type="o",main="Model ARIMA(3,1,0)(0,1,2)12 linearized")
...
```
#### Long term prediction linearized model
...
ts.plot(serie,tl2,tu2,pr2,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=ultim[1]+c(-1,+3),type="o",main="Model ARIMA(3,1,0)(0,1,2)12 Linearized")
abline(v=(ultim[1]-2):(ultim[1]+3),lty=3,col=4)
```


## Compare the original and linearized model

```{r}
ts.plot(serie,tl1,tu1,pr1,tl2,tu2,pr2,lty=c(1,2,2,1,2,2,1),col=c(1,4,4,2,3,3,6),xlim=ultim[1]+c(1,3),type="o",main="RenewUSA")
legend("topleft",c("ARIMA(3,1,0)(0,1,1)12","ARIMA(3,1,0)(0,1,2)12 linearized"),col=c(4,3),lty=1,lwd=2)
abline(v=ultim[1]+1:3,lty=3,col=4)
```

As we can observe, the predictions between the two models are very similar. The only difference lies in the confidence band, which has been reduced for the linearized model, which is a bit more assertive about its predictions.
...
row.names(resul)=c("ARIMA(3,1,0)(0,1,1)_12","ARIMA(3,1,0)(0,1,2)_12 Linearized")
resul
```
The linearized model is a better model according to AIC and BIC, but in this 2019 to 2020 prediction, the evaluation metrics (RMSE, MAE, RMSPE, MAPE) tell us that the non-linearized model gives slightly better values. For instance the RMSE of the ARIMA(3,1,0)(0,1,1)_12 is 27.05704, whilst for the ARIMA(3,1,0)(0,1,2)_12 Linearized is 27.68585, which is 0.6 points worse approximately.
